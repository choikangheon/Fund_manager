...

{
    ...
    public partial class Client : MetroForm
    {
        ...
        public int total_fund = 0;

        System.Timers.Timer autoTimer = new System.Timers.Timer();

        //telegram

        private static readonly string _token = "1796955439:AAHK7gstHcw22ej1QAM0_SW70jm0FtqEubY";
        private static readonly string _chid = "-1001248339755";
        ...
        public Client()
        {
            ...
            //autoFundStartButton.Click += autoFundStart; // 자동매매 시작
            ...
            else
            {
                ...
                for (int j = 0; j < realTimeFundList.Count; j++) //실시간후 저장 
                {
                    ...
                    else
                    {
                        ...
                        {
                            int lRet = 1;
                            lRet = buyOrder(realTimeFundList[j].종목코드, fundCount);
                            if (lRet == 0)
                            {
        ...
        // 주식 주문
        private int buyOrder(String 종목코드, int 수량)
        {
            int lRet = 1;
            //계좌 선택 여부 확인 
            ...
            {
                //주문하기 
                if (startBuyRadio.Checked == true)
                    lRet = axKHOpenAPI1.SendOrder("주식주문", GetScrNum(), this.accountComboBox.Text.Trim(), 1, 종목코드, 수량, 0, "03", "");
                else return lRet;
        ...
        public void buyBot(String 종목코드, int 보유수량)
        {
            buyOrder(종목코드, 보유수량);
        }
        ...
        private void OnReceiveRealCondition(object sender, AxKHOpenAPILib._DKHOpenAPIEvents_OnReceiveRealConditionEvent e)
        {
            ...
            if (e.strType == "I") // 조건검색 편입
            {
                ...
                for (int i = 0; i < realTimeAccountGridView.Rows.Count; i++) // 보유종목
                {
                ...
                if (0 <= tradingStrategyList[0].buyingItemCount)
                {
                    ...
                    if (flag != 1 && startBuyRadio.Checked == true )  //동일종목이 없으면
                    {
                        ...
                        if (count >= 10 /*&& total_fund <= 5*/)
                        {
                            ...
                            Console.WriteLine("구입수량>=10,total_fund<=5 Sucess");
                            realTimeAccountOnlabel.Checked = true;
                            tradingStrategyList[0].buyingItemCount += -1;
                        }
                        else
            ...
            if (e.sRQName == "계좌잔고개별")
            {
                ...
                try
                {
                    for (int i = 0; i < nCnt; i++)
                    {
                        ...
                        if (af.수익률 >= double.Parse(targetPrice.Text) || af.수익률 <= -15)
                        {
                            string sendMsg = "종목코드: " + af.종목코드 + "보유수량: " + af.보유수량 + "수익률: " + af.수익률 + "%";
                            // SendMessage(sendMsg);    //자동매도 수정
                            //Console.WriteLine("sendMessage telegram");
        ...
        private void sellOrderButton_Click(object sender, EventArgs e)
        {
        ...
        private void setStartBuy()
        {
            startBuyRadio.Checked = true;
        }
        ...
        /*
                public void Bot_Onmessage(object sender, Telegram.Bot.Args.MessageEventArgs e)
                {
                    ...
                    if (e.Message.Type == Telegram.Bot.Types.Enums.MessageType.Text)
                    {
                        ...
                        if (e.Message.Text.StartsWith("/sell"))
                        {
        ...
        public void init_ini(string tP)
        {
            ...
            WritePrivateProfileString("Setting", "targetPrice", tP, path);
            WritePrivateProfileString("Setting", "buyFlag", flag, path);
        ...
        public void read_ini()
        {
            ...
            GetPrivateProfileString("Setting", "targetPrice", "(NONE)", retId, 32, path);
            GetPrivateProfileString("Setting", "buyFlag", "(NONE)", retPw, 32, path);

            Console.WriteLine(retId);
            Console.WriteLine(retPw);

            targetPrice.Text = retId.ToString();

            if (retPw.ToString() == "On")
                startBuyRadio.Checked = true;
            else
                stopBuyRadio.Checked = true;
